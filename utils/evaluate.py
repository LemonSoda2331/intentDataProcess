...
def set_path(config):
    ...
    if config['emb_class'] == 'glove':
        args.data_path = os.path.join(args.data_dir, 'encode_utf-8text.json')
    else:
        if args.augmented:
    ...
    args.label_path = '../preprocess/' + 'label.txt'
    if args.augmented:
        args.test_path = os.path.join(args.data_dir, 'augmented.raw')
    else:
        args.test_path = os.path.join(args.data_dir, 'encode_utf-8text.json')
    args.vocab_path = '../models/chinese_roberta_L-12_H-256' + 'vocab.txt'
...
def evaluate(args):
    ...
    logger.info("[Elapsed Time(total_duration_time, average)] : {}ms, {}ms".format(total_duration_time,
                                                                                   total_duration_time / (
                                                                                               total_examples - 1)))
...
def get_folder(input_path, args):
    for file in os.listdir(input_path):
        text=[]
        with open(os.path.join(input_path,file),'r',encoding='utf-8') as f:
            print('\n-------start to load file-------')
            print(f'load file: {os.path.join(input_path,file)}')
            for line in json.loads(f.read()):
                text.append([re.sub(r'[。？，！；]$', '', line['text']), line['intent']])
            inference(args, text, file)     
        f.close()
    return text
    
def inference(args, texts, file_name):
    ...
    # prepare labels
    labels = config['labels']
    # logger.info("{}".format(texts))
    # result={'text':[],'predict_raw':[],'max_logits':[],'bert_output':[],'videoBotId':[],'pretext':[]}
    result={'text':[], 'intent': [], 'predict_raw':[], 'is_correct':[], 'max_logits':[],'bert_output':[]}
    ...
    if texts:
        ...
        for text in texts:
            ...
            # print(x, type(x))
            start_time = time.time()
            x = encode_text(config, tokenizer, text["text"], None)
            # x = to_device(x, args.device)
            logits = model(x)
            bert_output=model._compute_bert_embedding(x)
            ...
            y_raw = text["intent"]
            total_examples += 1
            if predicted_raw == y_raw:
                correct += 1
            else:
                print(f'{total_examples - correct}/{total_examples}, {text["text"]}, {y_raw}, {predicted_raw}')

            duration_time = float((time.time() - start_time) * 1000)
            # logger.info("{}\t{}\t{:.2f}ms".format(text, predicted_raw, duration_time))
            result['text'].append(text["text"])
            result['intent'].append(text["intent"])
            # result['videoBotId'].append(text["intent"])
            # result['pretext'].append(text[2])
            result['predict_raw'].append(predicted_raw)
            result['is_correct'].append(int(predicted_raw == y_raw))
            result['max_logits'].append(to_numpy(values)[0])
            result['bert_output'].append(bert_output[0][0].detach().cpu().numpy())

            text["predict_raw"] = predicted_raw
            text["max_logits"] = float(to_numpy(values)[0])
            text["bert_output"] = bert_output[0][0].detach().cpu().numpy().tolist()
            text['bert_predict_correct'] = int(predicted_raw == y_raw)
        result=pd.DataFrame(result)
        result.to_csv(f'../evaluate/{file_name}.csv',index=False)

        # f_out.write(text + '\t' + predicted_raw + '\t' +bert_output+'\n')
        # f_out.close()

        logger.info("[Inference Accuracy] : {:.4f}, {:5d}/{:5d}".format(correct / total_examples, correct, total_examples))
        ...
        with torch.no_grad(), open(args.test_path, 'r', encoding='utf-8') as f:
            ...
            for i, line in enumerate(json.loads(f.read())):
                start_time = time.time()
                # line = line.strip()
                # tokens = line.split('\t')
                sent_a = line['text']
                ...
                duration_time = float((time.time() - start_time) * 1000)
                if i != 0: total_duration_time += duration_time
                # logger.info("{}\t{}\t{}\t{:.2f}ms".format(text, y_raw, predicted_raw, duration_time))
                return None
                # logger.info("[Elapsed Time] : {}ms".format(duration_time))
        f_out.close()
        logger.info("[Accuracy] : {:.4f}, {:5d}/{:5d}".format(correct / total_examples, correct, total_examples))
        logger.info("[Elapsed Time(total_duration_time, average)] : {}ms, {}ms".format(total_duration_time,
                                                                                       total_duration_time / (
                                                                                                   total_examples - 1)))


def main_evaluate(intent_data, model_path):
    parser = argparse.ArgumentParser()

    parser.add_argument('--config', type=str, default='../configs/config-bert-cls.json')
    parser.add_argument('--data_dir', type=str, default='../tmp/')
    parser.add_argument('--model_path', type=str, default='../train/pytorch-model.pt')
    ...
    # for BERT
    parser.add_argument('--bert_output_dir', type=str, default='../results/roberta-checkpoint',
    ...
    # for Inference
    parser.add_argument('--enable_inference', default=True,
    ...
    # args.enable_inference = args.enable_inference.lower() == 'true'

    args.model_path = model_path

    if args.enable_inference:
        # intents_data_path = os.path.join(args.data_dir, 'intents.json')
        # with open(intents_data_path, 'r', encoding='utf-8') as f:
        #     intent_data= json.loads(f.read())
        # intents_data_path = '../preprocess/intents.json'
        texts = intent_data
        # for line in intent_data:
        #     texts.append([line['text'], line['intent']])
        texts = inference(args, texts ,'intents.csv')

        intent_data_evaluate_path = '../evaluate/' + "_evaluate.json"
        with open(intent_data_evaluate_path, 'w', encoding='utf-8') as f:
            f.write(json.dumps(texts, ensure_ascii=False))
        
        intent_data_evaluate_mismatch_path = '../evaluate/' + "_evaluate_mismatch.json"
        texts_mismatch = [obj for obj in texts if obj.get('intent')!= obj.get('predict_raw')]
        texts_mismatch = [{k: v for k, v in obj.items() if k != 'bert_output'} for obj in texts_mismatch]
        # sort texts_mismatch by max_logits
        texts_mismatch = sorted(texts_mismatch, key=lambda x: x['max_logits'], reverse=True)
        with open(intent_data_evaluate_mismatch_path, 'w', encoding='utf-8') as f:
            f.write(json.dumps(texts_mismatch, ensure_ascii=False))
        
        texts_mismatch_csv = pd.DataFrame(texts_mismatch)
        texts_mismatch_csv_path = '../evaluate/' + "_evaluate_mismatch.csv"
        texts_mismatch_csv.to_csv(texts_mismatch_csv_path, index=False)
...
#     print("predict time:{}s".format(total_time))