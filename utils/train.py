    ...
    for local_step, (x, y) in enumerate(epoch_iterator):
        ...
        if (local_step + 1) % args.gradient_accumulation_steps == 0:
            ...
            epoch_iterator.set_description(
                f"Process: {accelerator.process_index}, epoch: {epoch_i}, global_step: {global_step}, local_step: {local_step}, loss: {loss:.3f}, curr_lr: {curr_lr:.7f}")
            if args.eval_steps > 0 and global_step != 0 and global_step % args.eval_steps == 0:
                ...
                if (is_best or args.save_after_eval) and accelerator.is_main_process:
                    ...
                    if args.save_path and not args.hp_search_optuna:
                        ...
                        if config['emb_class'] not in ['glove'] and not (
                                config['emb_class'] == 'bart' and config['use_kobart']):
...
def evaluate(model, config, valid_loader, eval_device=None):
    ...
    auroc = AUROC(task='multiclass', num_classes=len(config['labels']))
    losses = []
    total_examples = 0
    correct = 0
    preds = None
...
def set_path(config):
    ...
    if config['emb_class'] == 'glove':
        if args.augmented:
            ...
            args.train_path = os.path.join(args.data_dir, 'train.json.ids')
        args.valid_path = os.path.join(args.data_dir, 'valid.json.ids')
    else:
        if args.augmented:
            args.train_path = os.path.join(args.data_dir, 'augmented.txt.fs')
        else:
            args.train_path = os.path.join(args.data_dir, 'encode_train.json')
        args.valid_path = os.path.join(args.data_dir, 'encode_train.json')
...
def train(args):
    ...
    with temp_seed(args.seed):
        ...
        # training
        early_stopping = EarlyStopping(logger, patience=args.patience, measure=args.measure,
                                       process_index=accelerator.process_index, verbose=1)
        local_worse_epoch = 0
        best_eval_measure = float('inf') if args.measure == 'loss' else -float('inf')
        for epoch_i in range(args.epoch):
            epoch_st_time = time.time()
            eval_loss, eval_acc, best_eval_measure = train_epoch(model, config, train_loader, valid_loader, epoch_i,
                                                                 best_eval_measure)
            if args.measure == 'loss':
                eval_measure = eval_loss
            else:
...
def hp_search_optuna(trial: optuna.Trial):
    ...
    with temp_seed(seed):
        ...
        for epoch in range(epochs):
            eval_loss, eval_acc, best_eval_measure = train_epoch(model, config, train_loader, valid_loader, epoch,
                                                                 best_eval_measure)
...
def get_params():
    ...
    parser.add_argument('--warmup_ratio', type=float, default=0.0,
                        help="Ratio for warmup over total number of training steps.")
    parser.add_argument('--patience', default=7, type=int, help="Max number of epoch to be patient for early stopping.")
    parser.add_argument('--save_path', type=str, default='../train/pytorch-model.pt')
    ...
    parser.add_argument('--measure', type=str, default='loss',
                        help="Evaluation measure, 'loss' | 'accuracy', default 'loss'.")
    parser.add_argument('--criterion', type=str, default='CrossEntropyLoss',
                        help="training objective, 'CrossEntropyLoss' | 'LabelSmoothingCrossEntropy' | 'MSELoss' | 'KLDivLoss' | 'IsoMaxLoss', default 'CrossEntropyLoss'")
    parser.add_argument('--local_rank', default=0, type=int)
    parser.add_argument('--use_fp16', action='store_true',
                        help="Use mixed precision training via torch.cuda.amp(inside Accelerate).")
    ...
    parser.add_argument('--bert_use_finetune_last', default=True)
    # for Optuna
...
def main_train(b):
    ...
    args.save_path = args.save_path + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + '-' + str(b) + '.pt'
